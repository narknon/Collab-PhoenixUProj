...
#include "AssetGenerationUtil.generated.h"
...
public:
	...
	FDeserializedProperty() : PropertyFlags(CPF_None), ArrayDim(1), BlueprintReplicationCondition(COND_None) {}
	
	FDeserializedProperty(const TSharedPtr<FJsonObject>& Object, class UObjectHierarchySerializer* ObjectSerializer);

	bool HasAllPropertyFlags(EPropertyFlags Flags) const {
		return (PropertyFlags & Flags) == Flags;
	}

	bool HasAnyPropertyFlags(EPropertyFlags Flags) const {
		return (PropertyFlags & Flags) != 0;
	}

	bool IsFunctionInputParam() const {
...
public:
	...
	FDeserializedFunction() : FunctionFlags(FUNC_None), bIsUberGraphFunction(false), bIsDelegateSignatureFunction(false), bIsCallingIntoUbergraph(false) {}
	...
	bool HasAllFunctionFlags(EFunctionFlags Flags) const {
